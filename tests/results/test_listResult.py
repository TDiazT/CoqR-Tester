...
from coqr.reports.results import NullResult, ErrorResult, NotImplementedResult, ImpossibleResult, UnknownResult, \
    FunctionResult, VectorResult, InvisibleResult, ListResult
...
class TestListResult(TestCase):
    ...
    def assert_list_equals(self, v1: List, v2: List):
        list_1 = ListResult(v1)
        list_2 = ListResult(v2)

        self.assertEqual(list_1.compare_to(list_2), Status.PASS)

    def assert_list_not_equals(self, v1: List, v2: List):
        vector_1 = VectorResult(v1)
        vector_2 = VectorResult(v2)

        self.assertEqual(vector_1.compare_to(vector_2), Status.FAIL)

    def test_compare_to_list(self):
        self.assert_list_equals([], [])
        self.assert_list_equals([1, 2, 3], [1, 2, 3])
        self.assert_list_equals(['1', '2', '3'], ['1', '2', '3'])
        self.assert_list_equals([True, True], [True, True])
        self.assert_list_equals([False], [False])
        self.assert_list_equals([False, True], [False, True])
        self.assert_list_equals([None, nan], [None, nan])
        self.assert_list_equals([1, [1]], [1, [1]])
        self.assert_list_equals([1, [1, [2, [3]]]], [1, [1, [2, [3]]]])
        self.assert_list_equals([1, [1, [2, [3]], None]], [1, [1, [2, [3]], None]])

    def test_failed_vector_comparison(self):
        self.assert_list_not_equals([], ['1'])
        self.assert_list_not_equals(['1'], [])
        self.assert_list_not_equals([1, 2], [1, 2, 3])
        self.assert_list_not_equals(['1', '2', '3'], ['1', '3', '2'])
        self.assert_list_not_equals([False], [True])
        self.assert_list_not_equals([True], [False])
        self.assert_list_not_equals([True, True], [True, False])
        self.assert_list_not_equals([False, True], [True, True])
        self.assert_list_not_equals(['NA', 'NaN'], ['NA', '1'])
        self.assert_list_not_equals(['NA', [1]], ['NA', '1'])
        self.assert_list_not_equals(['NA', [1, [2, None]]], ['NA', [1, [2]]])
