...
n <-   20; all(		 gamma(1:n) == cumprod(c(1,1:(n-1))))
n <-   50; all(abs(rErr( gamma(1:n), cumprod(c(1,1:(n-1))))) < 20*Meps)
...
all(is.nan(gamma(0:-47)))

n51 <- c(196793068630200, 229591913401900, 247959266474052)
abs(c(n51, rev(n51))- choose(51, 23:28)) <= 2
all(choose(0:4,2) == c(0,0,1,3,6))

gEuler <- 0.577215664901532860606512
abs(digamma(1) + gEuler) <   32*Meps
all.equal(digamma(1) - digamma(1/2), log(4), tolerance = 32*Meps)
n <- 1:12
all.equal(digamma(n),
         - gEuler + c(0, cumsum(1/n)[-length(n)]),tolerance = 32*Meps)
all.equal(digamma(n + 1/2),
          - gEuler - log(4) + 2*cumsum(1/(2*n-1)),tolerance = 32*Meps)
...
stopifnot(all.equal( digamma(x), psigamma(x,0), tolerance = 2*Meps),
          all.equal(trigamma(x), psigamma(x,1), tolerance = 2*Meps))
...
n <- 1000
x12 <- matrix(NA, 2,n); x12[,1] <- c(2.8,3)
